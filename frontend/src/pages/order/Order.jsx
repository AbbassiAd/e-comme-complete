...
const Order = () => {
 ...
 /*  return (
  ...
  <div class="bg-white shadow-md rounded-lg p-6">
    <h2 class="text-xl font-bold mb-4 text-gray-800">Shipping Details</h2>
    <div class="border-b border-gray-300 mb-4 pb-4">
      <p class="text-gray-700 mb-2"><strong>Order ID:</strong> {order?._id}</p>
      <p class="text-gray-700 mb-2"><strong>Name:</strong> {order?.user.firstname} {order?.user.lastname}</p>
      <p class="text-gray-700 mb-2"><strong>Email:</strong> {order?.user.email}</p>
      <p class="text-gray-700 mb-2"><strong>Address:</strong> {order?.shipping_address.address}, {order?.shipping_address.city} {order?.shipping_address.postalCode}, {order?.shipping_address.country}</p>
      <p class="text-gray-700 mb-2"><strong>Phone:</strong> {order?.shipping_address.phone}</p>
      <p class="text-gray-700 mb-2"><strong>Payment Method:</strong> {order?.payment_infos.paymentMethod}</p>
    </div>
  
    <h2 class="text-xl font-bold mb-4 text-gray-800">Order Summary</h2>
    <div class="border-b border-gray-300 mb-4 pb-4">
      <div class="flex justify-between mb-2">
        <span class="text-gray-700"><strong>Items</strong></span>
        <span class="text-gray-800">$ {order?.items_price}</span>
      </div>
      <div class="flex justify-between mb-2">
        <span class="text-gray-700"><strong>Shipping</strong></span>
        <span class="text-gray-800">$ {order?.shipping_price}</span>
      </div>
      <div class="flex justify-between mb-2">
        <span class="text-gray-700"><strong>Total</strong></span>
        <span class="text-gray-800">$ {order?.totalPrice}</span>
      </div>
    </div>
  
    {!order?.status.is_paid && (
      <button class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-full mb-4 focus:outline-none focus:shadow-outline" onClick={payOrder}>
        PAY ORDER
      </button>
    )}
  
    {userInfo && userInfo.isAdmin && order?.status.is_paid && !order?.status.is_livred && (
      <button class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded-full focus:outline-none focus:shadow-outline" onClick={deliverHandler}>
        Mark As Delivered
      </button>
    )}
  </div>

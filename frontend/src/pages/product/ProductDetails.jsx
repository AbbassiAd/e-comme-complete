...
<div className="relative bg-gray-200"><section className="bg-gray-200 w-full container mx-auto px-4 lg:px-0 py-8 lg:py-12 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
  <div className="relative bg-gray-200 lg:col-start-1 flex flex-col lg:flex-row lg:justify-start lg:items-start gap-8 p-8">
    {product?.images  && product?.images?.image1&& product?.images?.image2 &&  (
      <div id="image_secondaire" className="lg:mr-8 flex-shrink-0 flex flex-col justify-center items-center">
        <div className="flex flex-col gap-4">
          {Object.entries(product.images).map(([key, image], index) => (
            image && (
              <div key={index} className="rounded-lg  hover:shadow-xl">
                <img
                  src={image}
                  alt=""
                  className="w-16 h-16 border border-gray-300 rounded-md shadow-sm cursor-pointer hover:scale-105 transform transition duration-300 ease-in-out"
  ...
  <div className="bg-gray-200 flex flex-col">
    ...
    <div className="mt-6 mb-5 ">
          ...
          <div className="flex ">
            <button className=  {`${plusMinuceButton}`} onClick={minsQuantity} disabled={counter == 1} >−</button>
            <div className="flex bg-white h-8 w-8 cursor-text items-center justify-center border-t border-b active:ring-gray-500">
              {counter}
            </div>
            <button className={`${plusMinuceButton}`} onClick={plusQuantity} disabled={counter == product?.countInStock}> +</button>
          </div>
        </div>
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
      <div className="flex gap-4 mb-4 lg:mb-0">
        <button   onClick={() => addHandler(Number(counter),product)} className="flex items-center justify-center h-12 w-52 bg-violet-900 text-white duration-100 hover:bg-blue-800 rounded-lg">
          <BiShoppingBag className="w-6 h-6 mr-2" />
          Add to Cart
        </button >
        <button onClick={addToFavrites} className="flex items-center justify-center h-12 w-52 bg-amber-400 duration-100 hover:bg-yellow-300 rounded-lg">
